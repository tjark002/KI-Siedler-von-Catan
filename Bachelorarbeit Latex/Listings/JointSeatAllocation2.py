...
candidates, programs = algorithm4(candidates, programs, cat_change=cat_change)
spit_facts()
candidates, programs, cat_change = prepare_next_round(candidates, programs)
...
def spit_facts():
    ...
    for c in candidates:
        ...
        if c.p != 0:
            seat = seat + 1
            if c.isfemale:
                female_and_seat = female_and_seat+1
        if c.acceptance == 1:
            rejected = rejected + 1
        elif c.acceptance == 2:
            freezed = freezed + 1
            ...
            floated = floated + 1
    print("Kandidaten ohne Platz: ",no_seat-rejected, (no_seat-rejected)/len(candidates))
    print("Kandidaten mit Platz: ",seat, seat/(len(candidates)-rejected))
    print("Weibliche Kandidaten mit Platz: ",female_and_seat, female_and_seat/seat)
    print("Weibliche Kandidaten gesamt: ",female, female/(len(candidates)-rejected))
    print("Rejected: ",rejected, rejected/(len(candidates)))
    print("Freezed: ",freezed, freezed/(len(candidates)-rejected))
    print("Floated: ",floated, floated/(len(candidates)-rejected))
    
    free_seats = 0
    seats = 0
    for p in programs:
        seats = seats + p.capacity
        if len(p.wait_list) < p.capacity:
            free_seats = free_seats + (p.capacity - len(p.wait_list))
    print("Freie Plätze: ", free_seats)
    print("Plätze gesamt: ", seats)
